...
class FragmentNode():
    ...
    def template_screen(self, molecule: Molecule, store_data: bool=True) -> TemplateResult:
    ...
    def fragment_screen(self, fragment: str) -> bool:
    ...
    def match_unmapped_fragment(self, fragment: str) -> bool:
        return fragment.count('*') == self.dummy_smile.count('*')
    
    def match_mapped_fragment(self, fragment: str) -> bool:
        patt = re.compile('\[\*(.*?)]')
        frag_idxs = sorted([int(i[1:]) for i in patt.findall(fragment)])
        return frag_idxs == self.mapping_idxs
    
    def get_leaf_nodes(self) -> dict[str, FragmentNode]:
        outputs = {}
        stack = [self]
        while stack:
            current = stack.pop()
...
class FragmentLeafNode(FragmentNode):
    ...
    def match_and_map(self, fragment: str) -> list[str]:
        outputs = []
    ...
    def assemble(self, assembly_dict: dict) -> Molecule:
...
class ConstantFragmentNode(FragmentNode):
    ...
    def match_and_map(self, fragment: str) -> list:
        return []
